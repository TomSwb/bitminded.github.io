document.addEventListener('DOMContentLoaded', () => {
    ...
    fetch('lang-checkout/locales-checkout.json')
        .then(response => {
            if (!response.ok) {
        ...
        .catch(error => {
            window.logger?.error('Failed to load checkout translations:', error);
            revealContent();
        });
    ...
    function applyTranslations() {
        ...
        revealContent();
        document.dispatchEvent(new CustomEvent('checkoutTranslationsApplied', {
            detail: {
