document.addEventListener('DOMContentLoaded', function() {
    ...
    fetch('locales/privacy-locales.json')
        ...
        .catch(error => {
            window.logger?.error('Failed to load privacy translations:', error);
            // Still show content even if translation fails
            showContent();
        });

    function updateContent() {
        // Privacy page specific translations
        if (document.querySelector('h1[data-translate="privacy.title"]')) {
            document.querySelector('h1[data-translate="privacy.title"]').textContent = i18next.t('privacy.title');
        }
        if (document.querySelector('p[data-translate="privacy.subtitle"]')) {
            document.querySelector('p[data-translate="privacy.subtitle"]').textContent = i18next.t('privacy.subtitle');
        }
