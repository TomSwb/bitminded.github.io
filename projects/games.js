...
async function loadProjects() {
	const res = await fetch('games.json'); // gets the data in the json
	const projects = await res.json(); // what does it do?

	const grid = document.getElementById('games-grid'); // find the element in the html file with an id of games-grid
	...
	projects.forEach(project => {
		...
		card.className = 'games-cards'; //give it the class name to not loose my css
		...
		card.innerHTML = `
			...
			<img class="games-image" src="${escapeHtml(project.image)}" alt="${escapeHtml(project.alt)}">
			<p>${escapeHtml(project.description)}</p>
			<p>${escapeHtml(project.details)}</p>
			<p>${escapeHtml(project.engine)}</p>
			<a class="itch-button" href="${escapeHtml(project.itch_link)}" target="_blank" rel="noopener noreferrer">
			Try Now! </a>
