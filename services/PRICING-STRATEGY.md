# Services Pricing Strategy & Analysis

> **Last Updated:** January 2025  
> **Context:** One-person business in Vaud, Switzerland  
> **Target Market:** "Petite gens" (ordinary people, individuals, small projects)  
> **Capacity:** Full-time (35-45 hours/week)

---

## 📋 Table of Contents

1. [Current Pricing Structure](#current-pricing-structure)
2. [Volume Strategy Analysis](#volume-strategy-analysis)
3. [Modular Pricing Recommendations](#modular-pricing-recommendations)
4. [Market Assessment for Vaud](#market-assessment-for-vaud)
5. [Key Decisions & Rationale](#key-decisions--rationale)
6. [Revenue Scenarios](#revenue-scenarios)
7. [Implementation Notes](#implementation-notes)
8. [Future Considerations](#future-considerations)

---

## 💰 Current Pricing Structure

### Catalog Access

| Service | Current Price | Status |
|---------|--------------|--------|
| **Single Tool License** | CHF 2-25 (one-time, varies by tool) | ✅ Implemented |
| **All-Tools Membership** | CHF 5/month or CHF 55/year (Family: CHF 3.50/month or CHF 38.50/year) | ✅ Implemented |
| **Supporter Tier** | CHF 8/month or CHF 88/year (Family: CHF 5/month or CHF 55/year) | ✅ Implemented |

### Commissioning Services

| Service | Current Price | Duration | Status |
|---------|--------------|----------|--------|
| **Personal Project Intake** | CHF 20 (credited) | Quick call | ✅ Implemented |
| **Simple App** | CHF 350-750 | 1-2 weeks | ✅ Implemented |
| **Standard App** | CHF 850-1,250 | 2-4 weeks | ✅ Implemented |
| **Complex App** | CHF 1,350-1,950 | 6+ weeks | ✅ Implemented |
| **Commission a Feature** | CHF 20-150 | depending on difficulty | ✅ Implemented |

### Tech Support Services

| Service | Current Price | Reduced Fare | Duration | Additional Costs | Status |
|---------|--------------|--------------|----------|------------------|--------|
| **Confidence Session** | CHF 50 | CHF 35 | 1 hour | + travel | ✅ Implemented |
| **Guided Learning Bundle** | CHF 135 | CHF 95 | 3 sessions | + travel | ✅ Implemented |
| **Parent & Home Tech Session** | CHF 75 | CHF 50 | 90 minutes | + travel | ✅ Implemented |
| **Quick Tech Help** | CHF 30 | CHF 25 | 45 minutes | None (remote) | ✅ Implemented |
| **Friendly Tech Drop-In** | CHF 35 | CHF 30 | 1 hour | + travel + coffee 😉 | ✅ Implemented |
| **Home Visit Essentials** | CHF 90 | CHF 65 | 90 minutes | + travel | ✅ Implemented |
| **Device Procurement** | CHF 30/hour | CHF 20/hour | Variable | + device cost + travel | ✅ Implemented |

**Note:** Reduced fares available for seniors, students, and unemployed. Travel is CHF 0.50/km (min CHF 5, max CHF 40).

---

## 📊 Volume Strategy Analysis

### Strategy: Lower Price = Higher Volume

**Philosophy:** Focus on quantity of subscribers vs. amount per subscriber. Make catalog access accessible to "petite gens" (ordinary people), not just businesses.

### Revenue Targets

| Price Point | Subscribers Needed | Monthly Revenue | Annual Revenue (after tax ~25%) |
|-------------|-------------------|-----------------|--------------------------------|
| **CHF 5/month** | **1,000** | **CHF 5,000** | **CHF 45,000** |
| CHF 8/month | 625 | CHF 5,000 | CHF 45,000 |
| CHF 10/month | 500 | CHF 5,000 | CHF 45,000 |
| CHF 18/month | 278 | CHF 5,000 | CHF 45,000 |

### Current Implementation: CHF 5/month (All-Tools Membership)

**Rationale:**
- ✅ Most accessible to "petite gens" (very affordable)
- ✅ Lower barrier to entry
- ✅ Sustainable for Vaud living costs when combined with commissions and tech support

### Calculations (CHF 5/month - Current Implementation)

**Gross Revenue:**
- 1,000 subscribers × CHF 5 = CHF 5,000/month

**After Stripe Fees (3%):**
- CHF 5,000 - CHF 150 = CHF 4,850/month

**After Tax (~25%):**
- CHF 4,850 × 0.75 = CHF 3,638/month = CHF 43,650/year

**Verdict:** Sustainable for one person in Vaud, especially with commissions and tech support services. Note: Requires higher subscriber volume (1,000 vs 625) compared to CHF 8/month.

---

## 🎯 Project-Based Pricing Structure

### Commissioning: Project Complexity Scale

**Key Insight:** Apps can vary significantly in complexity. Simple apps can be built in 1-2 weeks, while complex apps may take 6+ weeks.

#### Simple App
- **Duration:** 1-2 weeks
- **Price:** CHF 350-750
- **Perfect for:** Personal tools, simple automations, focused utilities
- **Delivery:** 1-2 weeks

#### Standard App
- **Duration:** 2-4 weeks
- **Price:** CHF 850-1,250
- **Perfect for:** Small business tools, multi-feature apps
- **Delivery:** 2-4 weeks

#### Complex App
- **Duration:** 6+ weeks
- **Price:** CHF 1,350-1,950
- **Perfect for:** Multi-user systems, advanced integrations
- **Delivery:** 6+ weeks

### Current Pricing Breakdown

**Effective Hourly Rate:**
- Simple App (CHF 350-750 for 1-2 weeks): ~CHF 40-50/hour
- Standard App (CHF 850-1,250 for 2-4 weeks): ~CHF 40-50/hour
- Complex App (CHF 1,350-1,950 for 6+ weeks): ~CHF 40-50/hour
- **Assessment:** Appropriate for "petite gens" market (accessible, not premium agency pricing)

**Project-Based Pricing:**
- Simple App: CHF 350-750 (1-2 weeks) - Personal tools, simple automations, focused utilities
- Standard App: CHF 850-1,250 (2-4 weeks) - Small business tools, multi-feature apps
- Complex App: CHF 1,350-1,950 (6+ weeks) - Multi-user systems, advanced integrations
- Commission a Feature: CHF 20-150 (depending on difficulty) - New features for existing catalog tools

---

## 🏘️ Market Assessment for Vaud

### Target Market: "Petite Gens"

**Definition:** Ordinary people, individuals, small projects, hobby groups - not just businesses or enterprises.

**Characteristics:**
- Limited budgets compared to businesses
- Value accessibility and affordability
- Need personal touch and guidance
- Appreciate Swiss artisan quality at accessible prices

### Why Current Pricing Works

1. **Commissioning (CHF 40-50/hour effective rate):**
   - ✅ Accessible to individuals (CHF 350-750 for simple apps)
   - ✅ Not competing with premium agencies (CHF 80-150/hour)
   - ✅ Serves "petite gens" market effectively
   - ✅ Sustainable for full-time work (35-45 hours/week)
   - ✅ Simplified pricing structure (removed Discovery Intensive and Starter Customization)

2. **Tech Support Services (CHF 45-180):**
   - ✅ Accessible pricing for families and individuals
   - ✅ Fits Vaud market expectations
   - ✅ Complements commissioning services
   - ✅ Builds relationships and trust

3. **Catalog Access (Current: CHF 5/month - Implemented):**
   - ✅ Very accessible price for "petite gens" market
   - ✅ Volume strategy implemented (requires 1,000 subscribers for CHF 5k/month revenue)
   - ✅ Lower barrier = more subscribers
   - ✅ Family plans available at discounted rates (CHF 3.50/month)

### Capacity Analysis (35-45 hours/week)

**Per Month (160-180 hours):**
- 1 simple app (1-2 weeks) + 1 standard app (2-4 weeks) = ~90-120h
- 5-8 guidance sessions (prep + delivery) = 10-20h
- Admin/marketing/catalog work = 30-40h
- Buffer = 10-20h
- **Total: 140-180 hours/month** ✅ Fits capacity

**Revenue Potential:**
- 2 commissions/month: CHF 1.3k-2.0k (simple + standard)
- 5-8 guidance sessions: CHF 150-1,440 (CHF 30-180 per session)
- Catalog subscriptions: CHF 1.0k-5.0k (200-1,000 subscribers at CHF 5/month)
- **Total: CHF 2.5k-8.4k/month** = CHF 30k-100.8k/year

---

## 🔑 Key Decisions & Rationale

### 1. Pricing for "Petite Gens" (Not Premium Agencies)

**Decision:** Keep commissioning at CHF 40-50/hour effective rate (CHF 350-1,950 per project)

**Rationale:**
- Focus on accessibility, not premium positioning
- Serve individuals and small projects
- Build volume through accessibility
- Differentiate from expensive agencies (CHF 80-150/hour)
- Simplified pricing structure (project-based, not sprint-based)

### 2. Volume Strategy for Catalog

**Decision:** Lower catalog pricing to CHF 8/month (from CHF 18/month)

**Rationale:**
- Focus on quantity over price-per-subscriber
- Make it accessible to "petite gens"
- Lower barrier to entry = more subscribers
- 1,000 subscribers at CHF 5/month = CHF 5,000/month
- Family plans available at discounted rates (CHF 3.50/month for All-Tools, CHF 5/month for Supporter)

### 3. Project-Based Pricing for Commissioning

**Decision:** Simplified to project-based pricing (Simple/Standard/Complex apps)

**Rationale:**
- Apps vary significantly in complexity
- Simple apps can be built in 1-2 weeks
- Standard apps take 2-4 weeks
- Complex apps take 6+ weeks
- Project-based pricing is clearer and more accessible than sprint-based terminology
- Discovery is now included in Personal Project Intake (CHF 20, credited)
- Customization replaced by "Commission a Feature" for existing catalog tools

### 4. Tech Support Services Complement Commissioning

**Decision:** Keep tech support services at current pricing (CHF 45-180)

**Rationale:**
- Builds relationships and trust
- Generates commissioning leads
- Accessible pricing for families and individuals
- Fits Vaud market expectations

### 5. Deferred Services (Not Implemented)

**Decision:** Defer retainers, workshops, and consulting services

**Rationale:**
- Focus on core services first
- Build volume with catalog and tech support
- Add advanced services later if needed
- Keep scope manageable for one person

---

## 💵 Revenue Scenarios

### Scenario A: Simple Apps Focus

**Monthly:**
- 2-3 simple app commissions: CHF 700-2,250 (CHF 350-750 each)
- 5-8 guidance sessions: CHF 150-1,440 (CHF 30-180 per session)
- Catalog (200 subscribers at CHF 5): CHF 1,000
- **Total: CHF 1.85k-4.7k/month** = CHF 22.2k-56.4k/year

### Scenario B: Mixed Portfolio

**Monthly:**
- 1 standard + 1 simple commission: CHF 1.2k-2.0k
- 5-8 guidance sessions: CHF 150-1,440 (CHF 30-180 per session)
- Catalog (500 subscribers at CHF 5): CHF 2,500
- **Total: CHF 3.85k-4.9k/month** = CHF 46.2k-58.8k/year

### Scenario C: Tech Support-Heavy

**Monthly:**
- 1 simple commission: CHF 350-750
- 10-15 guidance sessions: CHF 300-2,700 (CHF 30-180 per session)
- Catalog (300 subscribers at CHF 5): CHF 1,500
- **Total: CHF 2.15k-4.95k/month** = CHF 25.8k-59.4k/year

### Scenario D: Volume Strategy Success

**Monthly:**
- 1 simple commission: CHF 350-750
- 5 guidance sessions: CHF 150-900 (CHF 30-180 per session)
- Catalog (1,000 subscribers at CHF 5): CHF 5,000
- **Total: CHF 5.5k-6.65k/month** = CHF 66k-79.8k/year

---

## 🛠️ Implementation Notes

### Current Pricing Implementation Status

#### Catalog Access (✅ Implemented)

**Current Prices (as of database seed):**
- All-Tools Membership: CHF 5/month or CHF 55/year (Family: CHF 3.50/month or CHF 38.50/year)
- Supporter Tier: CHF 8/month or CHF 88/year (Family: CHF 5/month or CHF 55/year)
- Single Tool License: CHF 2-25 (one-time, varies by tool)

**Note:** These prices reflect the volume strategy approach - lower barrier to entry, accessible to "petite gens", sustainable with commissions and tech support.

#### Commissioning (Simplified Structure)

**Current pricing structure:**
- Project-based pricing (Simple/Standard/Complex apps)
- CHF 450-1,950 per project (CHF 40-50/hour effective rate)
- Removed Discovery Intensive and Starter Customization services
- Discovery now included in Personal Project Intake (CHF 20, credited)
- Customization replaced by "Commission a Feature" for existing catalog tools
- Serves "petite gens" market effectively
- Sustainable for full-time work

#### Tech Support Services (No Changes Needed)

**Current pricing is appropriate:**
- CHF 45-180 is accessible
- Fits Vaud market expectations
- Complements commissioning services
- Builds relationships and trust

### Content Updates Needed

1. **Commissioning Page:**
   - ✅ Updated to project-based pricing (Simple/Standard/Complex apps)
   - ✅ Removed sprint terminology
   - ✅ Removed Discovery Intensive and Starter Customization
   - ✅ Discovery included in Personal Project Intake
   - ✅ Customization replaced by "Commission a Feature"

2. **Catalog Access Page:**
   - ✅ Pricing already implemented in database (CHF 5/month or CHF 55/year for All-Tools)
   - ✅ Supporter Tier: CHF 8/month or CHF 88/year
   - ✅ Family plans available at discounted rates
   - Ensure frontend displays match database values
   - Update FAQ if needed

3. **Business Model Document:**
   - Update catalog pricing section
   - Document volume strategy rationale
   - Update revenue projections

4. **Services Overview Page:**
   - Add complexity scale information
   - Emphasize accessibility for "petite gens"
   - Highlight modular pricing

---

## 🔮 Future Considerations

### Scaling Considerations

1. **Customer Acquisition:**
   - Need steady marketing/inbound for volume strategy
   - Low price helps word-of-mouth
   - Consider freemium or trial options

2. **Churn Management:**
   - Lower price may reduce commitment
   - Focus on value and engagement
   - Regular updates and new tools

3. **Support Scalability:**
   - More subscribers = more support questions
   - Automate where possible
   - Use community/forums

4. **Content Velocity:**
   - Regular new tools drive retention
   - Catalog must grow consistently
   - Consider user-generated content

### Potential Adjustments

1. **If Volume Strategy Succeeds:**
   - Consider adding more tiers (Individual, Family, Business)
   - Add more features to Supporter Tier
   - Consider annual-only pricing for deeper discount

2. **If Volume Strategy Struggles:**
   - Consider raising price to CHF 10/month
   - Focus on quality over quantity
   - Emphasize value proposition

3. **If Commissioning Demand Grows:**
   - Consider raising prices slightly (CHF 50-60/hour)
   - Focus on quality clients
   - Add premium tier for faster delivery

### Deferred Services (Future Implementation)

- **Build + Guide Retainer:** From CHF 450/month
- **Patron Bundle:** CHF 750/quarter
- **Family Plan Retainer:** CHF 160-320/month
- **Workshops / Consulting:** CHF 100-300/session or CHF 2k-20k/project

**Rationale:** Focus on core services first, add advanced services later if needed.

---

## 📝 Notes from Discussions

### Key Insights

1. **Pricing for "Petite Gens":**
   - Focus on accessibility, not premium positioning
   - Serve individuals and small projects
   - Build volume through accessibility

2. **Volume Strategy:**
   - Lower price = more subscribers
   - CHF 8/month is accessible and sustainable
   - 625 subscribers = CHF 5,000/month revenue

3. **Project-Based Pricing:**
   - Apps vary significantly in complexity
   - Simple apps can be built in 1-2 weeks
   - Standard apps take 2-4 weeks
   - Complex apps take 6+ weeks
   - Project-based pricing is clearer and more accessible
   - Discovery included in intake, customization replaced by feature commissions

4. **Capacity:**
   - Full-time work (35-45 hours/week)
   - Can handle 2-3 commissions/month
   - Can handle 5-10 tech support sessions/month
   - Catalog requires minimal maintenance time

5. **Market Fit:**
   - Vaud market appreciates accessibility
   - French-speaking market needs French support
   - "Petite gens" value personal touch
   - Swiss artisan quality at accessible prices

### Questions to Revisit

1. **Catalog Pricing:**
   - Should we implement CHF 8/month pricing?
   - Should we keep CHF 18/month for premium positioning?
   - Should we test both and see what works?

2. **Commissioning Pricing:**
   - Is CHF 40-50/hour sustainable long-term?
   - Should we raise prices as demand grows?
   - Should we add premium tier for faster delivery?

3. **Tech Support Services:**
   - Should we add more session types?
   - Should we create packages/bundles?
   - Should we add group sessions?

4. **Deferred Services:**
   - When should we implement retainers?
   - When should we add workshops/consulting?
   - Should we create bundles earlier?

---

## 📚 Related Documents

- [Business Model](../../docs/BUSINESS_MODEL.md)
- [Services Page Plan](./README.md)
- [Catalog Access Page](./catalog-access/index.html)
- [Commissioning Page](./commissioning/index.html)
- [Tech Support Page](./tech-support/index.html)

---

## 🔄 Changelog

### January 2025
- Initial pricing strategy analysis
- Volume strategy recommendation (CHF 8/month)
- Project-based pricing documentation
- Market assessment for Vaud
- Revenue scenario calculations
- Implementation notes for proposed changes
- **Removed Discovery Intensive and Starter Customization services**
- **Simplified commissioning to project-based pricing (Simple/Standard/Complex apps)**
- **Updated revenue scenarios to reflect new pricing structure**

---

**Last Updated:** January 2025  
**Next Review:** When implementing pricing changes or after 3 months of operation

