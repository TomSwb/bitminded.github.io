# Services Page Plan

- **Purpose:** Combine guidance/coaching offer, commissioning flow, and "How it works" into one cohesive services destination.
- **Top-Level Sections:**
  1. Overview hero reinforcing "We Build. We Guide." and clarifying who the services help.
  2. Guidance Services (Tech Confidence Coaching, Quick Tech Help, Family Tech Connection) with formats, pricing ranges, and booking CTA.
  3. Commission a Tool (process outline, deliverables, licensing terms, intake form link).
  4. How It Works (shared steps from discovery → session/project → follow-up rituals/notes).
- **Support Content:** FAQs, safety/privacy assurances, and quick link to the contact/support page.
- **Integration Notes:**
  - Booking/scheduling system (Calendly or custom form) for coaching sessions.
  - Intake form for commissions with Supabase backend or email routing.
  - Cross-links to catalog entries referenced in case studies.
- **Design Considerations:**
  - Mission toggle callout that routes between service types.
  - Mobile-first layout for session descriptions and CTAs.

---

## 📋 Documentation

### Pricing Strategy

See [PRICING-STRATEGY.md](./PRICING-STRATEGY.md) for comprehensive pricing analysis, volume strategy, and market assessment.

**Key Points:**
- **Target Market:** "Petite gens" (ordinary people, individuals, small projects)
- **Strategy:** Volume over premium pricing (lower price = more subscribers)
- **Catalog Pricing:** Proposed CHF 8/month (from CHF 18/month) for volume strategy
- **Commissioning:** CHF 40-50/hour (accessible, not premium agency pricing)
- **Guidance Services:** CHF 45-180 (fits Vaud market expectations)
- **Capacity:** Full-time (35-45 hours/week)

### Service Pages

- [Services Overview](./index.html) - Main services landing page
- [Catalog Access](./catalog-access/index.html) - Catalog pricing and tiers
- [Commissioning](./commissioning/index.html) - Commissioning process and pricing
- [Guidance Services](./guidance/index.html) - Guidance services and pricing
- [FAQ](./faq/index.html) - Frequently asked questions

---

## 🎯 Current Status

### Implemented Services

✅ **Catalog Access:**
- Single Tool License: CHF 25-60 (one-time)
- All-Tools Membership: CHF 18/month or CHF 180/year
- Supporter Tier: CHF 35/month

✅ **Commissioning:**
- Personal Project Intake: CHF 30 (credited)
- Simple App: CHF 450-750 (1-2 weeks)
- Standard App: CHF 850-1,250 (2-4 weeks)
- Complex App: CHF 1,350-1,950 (6+ weeks)
- Commission a Feature: CHF 20-150 (depending on difficulty - for new features on existing catalog tools)

✅ **Guidance Services:**
- Friendly Tech Drop-In: CHF 45 (30 minutes)
- Confidence Session: CHF 70 (1 hour)
- Quick Tech Help: CHF 90 (45 minutes)
- Family Tech Tune-Up: CHF 140 (90 minutes)
- Home Visit Essentials: CHF 120 + travel (90 minutes)
- Guided Learning Bundle: CHF 180 (3 sessions)
- Confidence Series: CHF 320 (4 sessions)
- Onsite Visit: CHF 180 + travel (variable)

### Proposed Changes

⚠️ **Catalog Access (To Be Implemented):**
- All-Tools Membership: CHF 8/month or CHF 80/year (from CHF 18/month)
- Supporter Tier: CHF 15/month (from CHF 35/month)
- Rationale: Volume strategy (lower price = more subscribers)

**Recent Changes:**
- ✅ Commissioning pricing restructured: Fixed project pricing based on complexity (Simple/Standard/Complex)
- ✅ Removed "sprint" terminology—using everyday language and project-based pricing
- ✅ Removed Discovery Intensive and Starter Customization—simplified pricing structure

---

## 📝 Notes

- All pricing is in CHF (Swiss Francs)
- Target market: "Petite gens" (ordinary people, individuals, small projects)
- Full-time capacity: 35-45 hours/week
- Location: Vaud, Switzerland
- See [PRICING-STRATEGY.md](./PRICING-STRATEGY.md) for detailed analysis and rationale
