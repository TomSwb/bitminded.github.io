...
serve(async (req) => {
  ...
  try {
    ...
    if (sendNotification && targetUser) {
      ...
      // If app_id is 'all' or invalid, link to account page
      // Otherwise, link directly to the tool: https://{slug}.bitminded.ch
      let notificationLink = '/account?section=subscriptions'
      if (entitlement.app_id && entitlement.app_id !== 'all' && typeof entitlement.app_id === 'string') {
        // Validate slug format (alphanumeric, hyphens, underscores)
        const slugPattern = /^[a-z0-9_-]+$/i
        if (slugPattern.test(entitlement.app_id)) {
          notificationLink = `https://${entitlement.app_id}.bitminded.ch`
        }
      }
