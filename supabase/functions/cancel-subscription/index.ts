...
serve(async (req) => {
  ...
  try {
    ...
    const { subscription_id, cancel_immediately = false, reason } = body
    ...
    if (!ownsSubscription) {
      await logError(
        ...
        'cancel-subscription',
        'auth',
        'User attempted to cancel subscription they do not own',
        { subscription_id, user_id: user.id },
        user.id,
        { subscription_id, cancel_immediately, reason },
        ipAddress
      )
      return new Response(
    ...
    try {
      ...
      await logError(
        ...
        { error: error.message, stack: error.stack, subscription_id, cancel_immediately },
        user.id,
        { subscription_id, cancel_immediately, reason },
        ipAddress
      )
    ...
    if (reason) {
      try {
        // Update purchase record with cancellation reason
        await supabaseAdmin
          .from('product_purchases')
        ...
        console.warn('⚠️ Failed to log cancellation reason:', error.message)
