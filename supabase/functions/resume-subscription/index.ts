...
serve(async (req) => {
  ...
  try {
    ...
    const { subscription_id, billing_cycle_anchor = 'now', proration_behavior = 'create_prorations' } = body
    ...
    // Validate billing_cycle_anchor
    if (!['now', 'unchanged'].includes(billing_cycle_anchor)) {
      return new Response(
        JSON.stringify({ error: 'Invalid billing_cycle_anchor. Must be one of: now, unchanged' }),
        { status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
      )
    }

    // Validate proration_behavior
    if (!['create_prorations', 'none'].includes(proration_behavior)) {
      return new Response(
        JSON.stringify({ error: 'Invalid proration_behavior. Must be one of: create_prorations, none' }),
        { status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
      )
    }
    ...
    if (!ownsSubscription) {
      await logError(
        ...
        'resume-subscription',
        'auth',
        'User attempted to resume subscription they do not own',
        { subscription_id, user_id: user.id },
        user.id,
        { subscription_id, billing_cycle_anchor, proration_behavior },
        ipAddress
      )
      return new Response(
    ...
    // Resume subscription
    let resumedSubscription: Stripe.Subscription
    try {
      // Use Stripe's resume method if available, otherwise update pause_collection to null
      try {
        resumedSubscription = await stripe.subscriptions.resume(subscription_id, {
          billing_cycle_anchor: billing_cycle_anchor as 'now' | 'unchanged',
          proration_behavior: proration_behavior as 'create_prorations' | 'none'
        })
      } catch (resumeError: any) {
        // If resume method doesn't exist, use update method
        if (resumeError.code === 'resource_missing' || resumeError.message?.includes('resume')) {
      ...
      await logError(
        ...
        { error: error.message, stack: error.stack, subscription_id, billing_cycle_anchor },
        user.id,
        { subscription_id, billing_cycle_anchor, proration_behavior },
        ipAddress
      )
      return new Response(
        JSON.stringify({ error: 'Failed to resume subscription', details: error.message }),
        { status: 500, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
      )
    }
    ...
    return new Response(
      JSON.stringify({
        ...
        current_period_end: resumedSubscription.current_period_end
      }),
      { status: 200, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
    )

  } catch (error: any) {
    console.error('❌ Unexpected error in resume-subscription:', error)
    const supabaseAdmin = createClient(
      Deno.env.get('SUPABASE_URL') ?? '',
      Deno.env.get('SUPABASE_SERVICE_ROLE_KEY') ?? '',
      { auth: { autoRefreshToken: false, persistSession: false } }
