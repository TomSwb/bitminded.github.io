 ...
 * Updates subscription plan (upgrade/downgrade)
 */
...
serve(async (req) => {
  ...
  try {
    ...
    const { subscription_id, new_price_id, proration_behavior = 'create_prorations' } = body

    // Validate required fields
    if (!subscription_id || !new_price_id) {
      return new Response(
        JSON.stringify({ error: 'Missing required fields: subscription_id and new_price_id are required' }),
        { status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
      )
    }

    // Validate proration_behavior
    if (!['create_prorations', 'none', 'always_invoice'].includes(proration_behavior)) {
      return new Response(
        JSON.stringify({ error: 'Invalid proration_behavior. Must be one of: create_prorations, none, always_invoice' }),
        { status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
      )
    }
    ...
    if (!ownsSubscription) {
      await logError(
        ...
        'update-subscription',
        'auth',
        'User attempted to update subscription they do not own',
        { subscription_id, user_id: user.id },
        user.id,
        { subscription_id, new_price_id, proration_behavior },
        ipAddress
      )
      return new Response(
    ...
    try {
      ...
      return new Response(
        JSON.stringify({ error: 'Failed to retrieve subscription' }),
        { status: 500, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
      )
    }
    ...
    try {
      ...
      await logError(
        ...
        { error: error.message, stack: error.stack, subscription_id, new_price_id, proration_behavior },
        user.id,
        { subscription_id, new_price_id, proration_behavior },
        ipAddress
      )
    ...
    // Format updated items for response
    ...
    console.error('❌ Unexpected error in update-subscription:', error)
    const supabaseAdmin = createClient(
      Deno.env.get('SUPABASE_URL') ?? '',
      Deno.env.get('SUPABASE_SERVICE_ROLE_KEY') ?? '',
      { auth: { autoRefreshToken: false, persistSession: false } }
